...
// 帳號數量規則
const accountNumberRegex = /^[1-9][0-9]{0,2}$/;
// 分行別規則
...
<template>
  <div class="pt-10">
    <div class="max-w-md mx-auto bg-white rounded-xl shadow-sm overflow-hidden md:max-w-xl">
      <div class="pt-10">
        ...
        <div class="mb-5 mt-10">
          <LabelInput id="startingAccount" :inputVal="labelForm.startingAccount" label="起始帳號"
            placeHolderText="01234567899" @changeInput="changeInput" @toValidate="validateForm" />
        </div>
        <div class="mb-3 mb-10">
          <LabelInput id="accountNumber" :inputVal="labelForm.accountNumber" label="帳號數量" placeHolderText="99"
            @changeInput="changeInput" @toValidate="validateForm" />
        </div>
    ...
    <div v-if="errorMessage"
      class=" mt-5 max-w-md mx-auto rounded-xl shadow-sm overflow-hidden md:max-w-xl flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
      role="alert">
    ...
    <!-- 顯示生成的完整帳號 -->
    <div v-else-if="resultArr.length > 0"
      class="mt-5 max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-xlw-50 md:max-w-xl mb-10">
